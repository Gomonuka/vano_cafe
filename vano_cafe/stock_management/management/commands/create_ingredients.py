...
#Function, that creates and adds products to the database, if there are none (initial product creation)
            ...
            for product_name, details in products.items():
                ...
                for ingredient, qty in ingredients.items():
                    if not Product.objects.filter(name=ingredient).exists():
                        Product.objects.create(name=ingredient, category="ingredient", unit="pc", price=0.01, stock=1000, low_stock_threshold=100)
